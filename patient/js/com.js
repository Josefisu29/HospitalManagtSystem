...
//     import { getFirestore, doc, getDoc, setDoc, updateDoc, onSnapshot, Timestamp, serverTimestamp } from "[invalid url, do not cite]
//     import { showLoading, hideLoading, showError, showSuccess, toggleOnlineMode, toggleOfflineMode, syncLocalData } from "../js/common.js";
    ...
    function populateForm(data) {
      ...
      Object.entries(data).forEach(([key, value]) => {
        ...
        if (el) {
          el.value = value?.toDate ? value.toDate().toISOString().split("T")[0] : value || "";
        }
      });
    }
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const payload = Array.from(form.elements)
          ...
          .reduce((o, el) => {
            o[el.id] = el.type === "date" && el.value ? Timestamp.fromDate(new Date(el.value)) : el.value;
            return o;
          }, {});
        ...
        showSuccess(navigator.onLine ? "Profile saved successfully" : "Profile saved locally. Will sync when online.");
      } catch (error) {
